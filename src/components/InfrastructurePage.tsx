...
const InfrastructurePage: React.FC<InfrastructurePageProps> = ({
  ...
  const getServicesForProvider = () => {
    if (selectedProvider === "all") {
      ...
      const allServices = Object.entries(servicesByProvider).flatMap(
        ([providerId, services]) =>
    ...
    return (
      servicesByProvider[
        selectedProvider as keyof typeof servicesByProvider
      ]?.map((service) => ({ ...service, providerId: selectedProvider })) || []
    );
  };

  const filteredServices = getServicesForProvider().filter((service) => {
    const matchesSearch =
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Browse and request cloud services from{" "}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {cloudProviders.map((provider) => (
                ...
                >
                  <div
                    className={`w-3 h-3 rounded-full mr-2 ${getProviderColor(
                      provider.id
                    )}`}
                  />
                  {provider.name}
