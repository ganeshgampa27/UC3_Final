                ...
                <div key={item.title}>
                  {item.subItems ? (
                    <>
                      {/* Parent menu button */}
                      <button
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-md text-sm font-medium transition-colors ${getNavCls(isCostCenterActive())}`}
                        onClick={() => {
                          if (isCollapsed) {
                            setIsCollapsed(false);
                          } else {
                      ...
                      {expanded[item.title] && !isCollapsed && (
                        <div className="ml-4 mt-1 space-y-1">
                          {item.subItems.map((subItem) => (
                            <button
                              key={subItem.title}
                              className={`w-full flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${getNavCls(isActive(subItem.tab))}`}
                              onClick={() => handleNavClick(subItem.tab)}
                            >
                              <subItem.icon className="w-5 h-5 ml-4" />
                              <span className="ml-3">{subItem.title}</span>
                    ...
                    /* Regular menu item */
                    <button 
                      className={`w-full flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${getNavCls(isActive(item.tab))}`}
                      onClick={() => handleNavClick(item.tab)}
                    >
                      <item.icon className="w-5 h-5" />
                      {!isCollapsed && <span className="ml-3">{item.title}</span>}
