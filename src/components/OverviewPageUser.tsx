...
const OverviewPageUser= () => {
  ...
  // Filter requests based on cloudProvider
  const filteredRequests = cloudProvider === "all" 
    ? requests 
    : requests.filter(
        (req) => req.Cloud.toLowerCase() === cloudProvider.toLowerCase()
      );
  ...
  const getEmployeeRequestStats = (): EmployeeRequestStats => {
    return {
      ...
      successfulApprovals: filteredRequests.filter((r) => r.Status === 'approved').length,
      pendingApprovals: filteredRequests.filter((r) => r.Status === 'pending').length,
      rejectedApprovals: filteredRequests.filter((r) => r.Status === 'rejected').length,
      appliedRequests: filteredRequests.filter((r) => r.Status === 'applied').length,
    };
  };
  ...
  const employeeStats = getEmployeeRequestStats();
  const recentRequestsLimited = filteredRequests
          ...
          <div className="space-y-3">
            {recentRequestsLimited.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>
                  {cloudProvider !== "none" 
                    ? `No requests found for ${cloudProvider.toUpperCase()}` 
                    : "No requests found"
                  }
                </p>
              </div>
